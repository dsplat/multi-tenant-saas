...
const fetchOverview = async () => { try { const r = await axios.get('/api/v1/admin/billing/credits/overview'); overview.value = r.data.data || r.data } catch {} }
...
const handleBatchRecharge = async () => {
  ...
  try {
    ...
    const r = await axios.post('/api/v1/admin/billing/credits/batch-recharge', { tenant_ids: ids, amount: rechargeForm.value.amount, remark: rechargeForm.value.remark })
