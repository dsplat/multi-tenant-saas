 ...
 * 商品类型见 Product::TYPES（physical / virtual / course / activity / points_goods / package）；
...
class ProductServiceProvider extends ModuleServiceProvider
{
    ...
    private function registerTools(): void
    {
        ...
        $registry->register('create_product', 'Create Product', 'Create a product in the unified product catalog', CreateProductHandler::class, ['type' => 'object', 'properties' => ['name' => ['type' => 'string', 'description' => '商品名称'], 'description' => ['type' => 'string', 'description' => '商品描述'], 'category_id' => ['type' => 'integer', 'description' => '分类ID（可选）'], 'price' => ['type' => 'number', 'description' => '售价'], 'market_price' => ['type' => 'number', 'description' => '市场价（可选）'], 'stock' => ['type' => 'integer', 'description' => '库存'], 'status' => ['type' => 'string', 'description' => '状态（默认 draft）'], 'type' => ['type' => 'string', 'description' => '类型：' . implode('/', Product::TYPES)], 'sale_mode' => ['type' => 'string', 'description' => '售卖模式（可选）']], 'required' => ['name']], 'product', 'L2');
