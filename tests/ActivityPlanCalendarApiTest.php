 ...
 */
class ActivityPlanCalendarApiTest extends TestCase
{
    protected array $uses = [ActivityPlanModule::class, RbacModule::class];

    private const API = '/api/v1/tenant/activity-plan';
    ...
    private function manualPlan(string $name = '测试活动'): ActivityPlan
    {
        return ActivityPlan::create([
            'tenant_id' => 1001,
            'plan_doc' => ['schema' => 'activity.plan/v1', 'manual' => true, 'title' => $name, 'phases' => []],
    ...
    public function test_tasks_index_filters_by_plan_and_range(): void
    {
        ...
        ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $planA->plan_id, 'task_key' => 'a1', 'title' => 'A1', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-10 10:00:00', 'action' => [], 'status' => 'pending']);
        ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $planA->plan_id, 'task_key' => 'a2', 'title' => 'A2', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-09-10 10:00:00', 'action' => [], 'status' => 'pending']);
        ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $planB->plan_id, 'task_key' => 'b1', 'title' => 'B1', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-12 10:00:00', 'action' => [], 'status' => 'pending']);
    ...
    {
        $plan = $this->manualPlan();
        $task = ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $plan->plan_id, 'task_key' => 't1', 'title' => '待办', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-10 10:00:00', 'action' => [], 'status' => 'pending']);
    ...
    {
        $plan = $this->manualPlan();
        $task = ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $plan->plan_id, 'task_key' => 't2', 'title' => '改期', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-10 10:00:00', 'action' => [], 'execution_mode' => 'auto', 'status' => 'pending']);
    ...
    {
        $plan = $this->manualPlan();
        $task = ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $plan->plan_id, 'task_key' => 't3', 'title' => '已完成', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-10 10:00:00', 'action' => [], 'status' => 'done']);
    ...
    {
        $plan = $this->manualPlan();
        $task = ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $plan->plan_id, 'task_key' => 'd1', 'title' => '删我', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-10 10:00:00', 'action' => [], 'status' => 'pending']);

        $this->actingAs($this->admin, 'sanctum')
            ->deleteJson(self::API . "/tasks/{$task->task_id}")
            ->assertOk();

        $this->assertDatabaseMissing('activity_tasks', ['task_id' => $task->task_id]);
    }

    public function test_delete_manual_plan_cascades_tasks(): void
    {
        $plan = $this->manualPlan();
        ActivityTask::create(['tenant_id' => 1001, 'plan_id' => $plan->plan_id, 'task_key' => 'c1', 'title' => '级联', 'trigger_type' => 'at_time', 'scheduled_at' => '2026-08-10 10:00:00', 'action' => [], 'status' => 'pending']);

        $this->actingAs($this->admin, 'sanctum')
            ->deleteJson(self::API . "/plans/{$plan->plan_id}")
            ->assertOk();

        $this->assertDatabaseMissing('activity_plans', ['plan_id' => $plan->plan_id]);
        $this->assertDatabaseMissing('activity_tasks', ['plan_id' => $plan->plan_id]);
    }
    ...
    public function test_delete_dsl_plan_rejected(): void
    {
        $plan = ActivityPlan::create([
            'tenant_id' => 1001,
            'plan_doc' => ['schema' => 'activity.plan/v1', 'title' => 'DSL 计划', 'phases' => []],
            'status' => ActivityPlan::STATUS_SCHEDULED,
            'created_by' => $this->admin->operator_id,
        ]);
