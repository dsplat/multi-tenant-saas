...
/**
 * ActivityTaskExecutor Phase 2 单测
 *
 * - task_chain 执行（forceL2 + 循环 advance）
 * - agent_react 执行（HeadlessAgentService 桩化）
 */
class ActivityPlanExecutorPhase2Test extends TestCase
{
    protected array $uses = [ActivityPlanModule::class, TaskChainModule::class, AgentModule::class];
    ...
    public function test_task_chain_executes_and_completes(): void
    {
        ...
        $task->refresh();
        $this->assertSame(ActivityTask::STATUS_DONE, $task->status);
    ...
    public function test_task_chain_with_missing_chain_key_fails(): void
    {
        ...
        $task->refresh();
        $this->assertSame(ActivityTask::STATUS_FAILED, $task->status);
    ...
    public function test_recurring_expands_to_multiple_at_time_tasks(): void
    {
        ...
        $plan->refresh();
        $this->assertSame(ActivityPlan::STATUS_SCHEDULED, $plan->status);

        // 应展开为 daily_sms_0, daily_sms_1, daily_sms_2
        $tasks = ActivityTask::where('plan_id', $plan->plan_id)->orderBy('task_key')->get();
    ...
    public function test_on_event_triggers_matching_pending_tasks(): void
    {
        $plan = ActivityPlan::create([
            'tenant_id' => self::TENANT,
            'plan_doc' => ['schema' => 'activity.plan/v1', 'title' => 'Event Test', 'phases' => []],
        ...
        $task = ActivityTask::create([
            ...
            'title' => '订单支付后发短信',
            'trigger_type' => ActivityTask::TRIGGER_ON_EVENT,
        ...
        // 模拟事件触发
        $subscriber = $this->app->make(ActivityPlanEventSubscriber::class);
    ...
    public function test_on_event_does_not_trigger_other_tenant_tasks(): void
    {
        ...
        $task->refresh();
        // 不应触发
        $this->assertSame(ActivityTask::STATUS_PENDING, $task->status);
    }
    ...
    public function test_on_event_require_confirm_sets_awaiting(): void
    {
        $plan = ActivityPlan::create([
            'tenant_id' => self::TENANT,
            'plan_doc' => ['schema' => 'activity.plan/v1', 'title' => 'Event Confirm Test', 'phases' => []],
            'status' => ActivityPlan::STATUS_RUNNING,
            'created_by' => 0,
        ]);
        ...
        $task->refresh();
        $this->assertSame(ActivityTask::STATUS_AWAITING_CONFIRM, $task->status);
    }

    // ── Helpers ──

    private function makeExecutor(?HeadlessAgentService $headless = null): ActivityTaskExecutor
    {
        $headless ??= $this->createMock(HeadlessAgentService::class);

        return new ActivityTaskExecutor(
    ...
    private function createTask(array $action): ActivityTask
    {
        $plan = ActivityPlan::create([
            'tenant_id' => self::TENANT,
            'plan_doc' => ['schema' => 'activity.plan/v1', 'title' => 'Test', 'phases' => []],
            'status' => ActivityPlan::STATUS_RUNNING,
            'created_by' => 0,
        ]);
