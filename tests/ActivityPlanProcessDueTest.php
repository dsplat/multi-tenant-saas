...
/**
 * activity_plan:process-due：到点触发 auto 工具执行、依赖未满足不触发、
 * require_confirm 置 awaiting_confirm、失败置 failed
 */
class ActivityPlanProcessDueTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config(['ai.activity_plan.enabled' => true]);
    }

    private function createPlan(string $status = 'scheduled'): ActivityPlan
    {
        return ActivityPlan::create([
            'tenant_id' => 1001,
            'plan_doc' => ['schema' => 'activity.plan/v1', 'title' => 'T', 'phases' => []],
            'status' => $status,
            'created_by' => 1,
        ]);
    }
    ...
    public function test_auto_task_triggers_when_due(): void
    {
        ...
        $this->artisan('activity_plan:process-due')->assertSuccessful();
    ...
    {
        config(['ai.activity_plan.enabled' => false]);
