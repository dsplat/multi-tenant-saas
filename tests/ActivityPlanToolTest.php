...
/**
 * 活动排期三工具单测（docs/event-plan.md Phase 1：B3 + B4 + B5）
 *
 * - draft：任务化长工具——工具毫秒级提交 AiTask 返回 await_task，
 *   重模型生成在 ActivityPlanDraftTaskHandler（queue sync 下同步执行）
 * - commit：编译验证 + 状态流转
 * - status：查询返回结构
 */
class ActivityPlanToolTest extends TestCase
{
    ...
    public function test_commit_compiles_plan_and_creates_tasks(): void
    {
        ...
        $this->assertSame(ActivityTask::STATUS_PENDING, $tasks->first()->status);
    }
    ...
    public function test_commit_falls_back_to_latest_draft_result_when_plan_id_invalid(): void
    {
        ...
        $tool = $this->app->make(ActivityPlanCommitTool::class);
        $result = $tool([
            'plan_id' => 1, // 幻觉编号
            'anchor_times' => ['event.starts_at' => '2026-09-01 09:00'],
        ], self::TENANT);
    ...
    public function test_status_returns_plan_and_tasks(): void
    {
        ...
        ActivityTask::create([
            ...
            'action' => ['type' => 'human'],
            'status' => ActivityTask::STATUS_AWAITING_CONFIRM,
        ]);

        $tool = new ActivityStatusTool;
    ...
    /**
     * 任务化接线：注册 activity_plan_draft 任务 handler + 绑定 LLM 桩
    ...
    private function registerStubTool(string $slug): void
    {
        $this->app->make(ToolRegistryContract::class)->register(
            ...
            ActivityPlanStubToolHandler::class,
            ['type' => 'object', 'properties' => []], 'test', 'L1',
        );
    }
}

class ActivityPlanStubToolHandler implements ToolHandlerContract
{
