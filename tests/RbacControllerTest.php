...
class RbacControllerTest extends TestCase
{
    ...
    {
        $role = $this->rbacService->createRole($this->tenantId, 'test_role', '测试角色');
    ...
    {
        $this->rbacService->createRole($this->tenantId, 'existing_role', '已存在角色');

        $this->expectException(\RuntimeException::class);
        $this->rbacService->createRole($this->tenantId, 'existing_role', '重复角色');
    }
    ...
    public function test_get_role_permissions(): void
    {
        ...
        $permissions = $this->rbacService->getRolePermissions($role->role_id);
    ...
    {
        // 测试权限检查（可能需要系统角色种子）
        $result = $this->rbacService->check('tenant.view');
